...
# Create random perturbations in the Z direction to simulate a noisy terrain
p_fail = 0.01 # probability of failure
z_perturb = 1 # meter
z_noise = np.random.uniform(0,1,num_points)
z_noise = np.where(z_noise < p_fail, z_perturb, 0)
...
# Display
cv2.imshow('[1] Composite Safety Maps', composite_map)
cv2.waitKey(10)

#################
# Modify PCD buffer by zooming in on 2D region of smaller radius
#################
new_rad_pointcloud = 80 # meters
...
# Display
cv2.imshow('[2] Composite Safety Maps', composite_map)
cv2.waitKey(100)
